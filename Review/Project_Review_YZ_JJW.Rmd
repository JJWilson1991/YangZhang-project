...
The project has quite a detailed background, but it does not flow very well and seems quite disjointed. The in depth discussion of the lifecycle of the parasites would benefit from some form of diagram to help visualise the process. More effort to link the relevance of this study to potential application would provide stronger motivation for the research.
...
There is a broad overall hypothesis related to the genes in question being related to parasite development. Some more specific hypotheses stemming from this broad concept would be a nice inclusion and allow the reader to track more easily what each part of the analysis is trying to demonstrate. For instance, the broad hypothesis is that the genes are related to development, this could be followed by narrower hypotheses suggested how the writer thinks they may be related and what sort of pattern they expect to see.
...
The data is provided in the form of excel spreadsheets. The source of the data is also provided.  There is no code book and it is difficult to know what the numbers are actually referring to other than "level of expression" so some explantation of whta the data is actually referring to would be helpful. It is explained slightly more withion the data cleaning RMD file but some explanation in the manuscript would help too.
